...
class Flight extends DB
{
    public function getAllFlight(){
        $sql = "SELECT fl.id, fl.flight_number, fl.image, fl.total_passengers, fl.description, ai.name FROM flights AS fl INNER JOIN airlines as ai ON fl.id = ai.id";
    ...
    public function getOneFlight($id){
        $sql = "SELECT fl.flight_number,fl.image, fl.total_passengers, fl.description, ai.name FROM flights AS fl INNER JOIN airlines AS ai ON fl.id = ai.id WHERE fl.id =".$id;
